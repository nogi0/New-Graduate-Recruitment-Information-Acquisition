...
`localStorage`はブラウザのキャッシュをクリアすると消えるため、ブックマークタブに
バックアップ用のツールを用意している。

- 📋 コピー / ⬇️ ダウンロード: ブックマーク済み記事のタイトル・URL・コメントをMarkdown形式で
  クリップボードにコピー、または`.md`ファイルとしてダウンロードする
- 💬 Discordに送信: 同じ内容をDiscordの任意のチャンネルに送信する。事前に「⚙️ Discord設定」から
  自分のDiscordサーバーで発行したWebhook URLを保存しておく必要がある。
  **このURLはブラウザのlocalStorageにのみ保存され、リポジトリには一切含まれない**
  （publicリポジトリにWebhook URLをコミットすると誰でも見られる状態になり、
  第三者に勝手にメッセージを送信されてしまうため）
...
- 会員登録・購読しないと読めないことが多い媒体（日経グループ、東洋経済オンライン、
  ダイヤモンド・オンラインなど）はRSSの媒体名で自動的に除外する。除外したい媒体が増えたら
  `scripts/collect_news.py` の `PAYWALL_SOURCE_KEYWORDS` に追記する。ただし媒体名ベースの
  判定なので、無料記事と有料記事が混在する媒体（note等）は完全には対応できない
